...
addMap_selectCity = function(map, countryName, bound_id){
  bound = getBoundary(countryName) %>% filter(ID_2==bound_id)
  layerName = "selected city"
  labels <- paste0("<h4 style='text-align: center;'>",bound$NAME_2,"</h4>") %>% lapply(htmltools::HTML)
...
addMap_popName = function(map, countryName, popName){
  ...
  map %>%
    # main_map %>%
    addRasterImage(popRaster, colors = pal, opacity = 0.5, group = country_pop_layer_name , options = tileOptions(pane = "pop")
    ) %>%
...
getRankPlotData = function(countryName, themeName, popName){
  ...
  if(themeName=="PM2.5"){
    ...
    rename_if(grepl("_num", names(.)), ~gsub("_prec","",.x)) %>% 
      ...
      replace(is.na(.), 0) #%>% replace(is.nan(.), 0)
...
getRankTable = function(rankPlotData, themeName){
  field_show = c("name", "ID_2","group_all",
  ...
  names(d) <- c("city name", "ID_2",tb %>% pull(varName))
