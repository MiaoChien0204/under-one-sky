...
# MAP ---------------------------------------------------------------------
    ...
    observeEvent({
      ...
      #rankPlotData = getRankPlotData(countryName, themeName="AQ Station", popName="all")
      ...
      output$rank_table <- DT::renderDataTable({
        d = getRankTable(rankPlotData, themeName) %>% dplyr::select(-ID_2)
        ...
        DT::datatable(d, 
          ...
          observeEvent(input$rank_table_rows_selected, {
            ...
            if(!is.null(row)){
              bound_id = getRankTable(rankPlotData, themeName) %>% .[rowID,] %>% pull(ID_2)  
              message(bound_id)
